...
public class PipelineExec {
     ...
     */
    public PipelineExec(Query query, ReactionListRetriever retriever, LayoutAlgorithm layoutAlg, LayoutRenderer layoutRend, Integer sbmlLevel, Integer sbmlVersion, OutputStream imageOutputStream) {
        this.query = query;
    ...
    public void run() {
        Reconstruction recons = retriever.getReactionsAsReconstruction(query);
        ...
        /**
         * Should we produce directly a layout image, or should we keep the intermediate layouted SBML?
         * Probably for the first versions it will be useful as a debugging/check step.
         */
        SBMLDocument docWLayout = layoutAlg.getLayoutedSBML(doc);
        layoutRend.produceRender(docWLayout, imageOutputStream);
    }
}
